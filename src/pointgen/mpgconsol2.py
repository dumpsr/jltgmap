...
# Clustering radius in meters for final consolidation
CLUSTER_RADIUS_M = 100

# For the special bus-stop filter:
# Remove bus stops within 250 meters of an already-kept stop,
# unless they are within 100m AND collectively serve multiple lines.
BUS_TOO_CLOSE_RADIUS_M = 400
BUS_MULTI_LINE_RADIUS_M = 100

OUTPUT_KML = "./out/lmtdfinal_consolidated_stations.kml"
...
def filter_close_bus_stops(bus_stops_df, dist_remove=250, dist_multiline=100):
    ...
    UNLESS the stop is within 'dist_multiline' meters and they collectively serve multiple lines.
    ...
    kept_records = []  # will store rows (dict) we keep

    # We can iterate in the order the stops_df is given, or sort by stop_id, etc.
    # For simplicity, keep the existing order:
    for _, row in bus_stops_df.iterrows():
        s_lat = row["stop_lat"]
        s_lon = row["stop_lon"]
        s_routes = row["routes"] if isinstance(row["routes"], set) else set()

        # Check against all stops we have already kept
        should_keep = True
        for kept in kept_records:
            ...
            if dist < dist_remove:
                ...
                # But check the special exception: if dist < dist_multiline and union of routes is > 1
                union_routes = s_routes.union(k_routes)
                if dist < dist_multiline and len(union_routes) > 1:
                    # We do NOT remove because they collectively serve multiple lines
                    # => keep it
                    should_keep = True
                    # We can break, since we found a reason to keep it
                    break
                else:
                    # We'll remove it
                    should_keep = False
    ...
    # Rebuild a DataFrame of the kept stops
    kept_df = pd.DataFrame(kept_records)
    return kept_df

##############################################################################
# 8) CONSOLIDATE (CLUSTER) STOPS WITHIN RADIUS
##############################################################################
...
def main():
    ...
    ########################################################################
    # NEW STEP: Filter out bus stops that are <250m from an already-kept bus stop,
    # unless they are <100m with collectively multiple lines.
    ########################################################################
    # We'll only apply this to the bus stops that passed the above logic
    # i.e., those in bs_filtered with route_type=3 or that appear among bus_stops_qualified
    # We'll do a quick filter: which are actually "bus stops" by checking if they had route_type=3?
    # Actually we only know if they've got bus routes. Let's just do it to *all in bus_stops_qualified*
    # Then union the result with the non-bus portion from bs_filtered if needed.

    # Identify which are bus stops by checking if they have at least one route_type=3 route.
    # But simpler is: we know these are from the "Bus/Subway" feed. 
    # If their route set has at least one bus route_type=3, we treat them as bus stops. 
    # But we didn't store route_type in 'routes'. We'll assume all 'routes' from this feed that made it are
    # potentially bus or non-bus. For clarity, let's just apply the 250m filter to *all* in bs_filtered that
    # are in 'bus_stops_qualified'. Because we only want to prune those physically close, that actually are bus stops.

    # We'll isolate the subset that truly got in by the bus pass. We'll consider them "candidate bus stops".
    # Non-bus stops from this feed are unaffected by the new filter.

    # a) "candidate_bus" = those whose 'stop_id' in bus_stops_qualified (meaning it satisfied bus freq)
    #    intersection with route_type=3 is more exact, but let's do the simple approach:
    candidate_bus = bs_filtered[bs_filtered["stop_id"].isin(bus_stops_qualified)].copy()
    # b) The rest are non-bus from this feed
    nonbus_part   = bs_filtered[~bs_filtered["stop_id"].isin(bus_stops_qualified)].copy()

    # Apply the 250m filter to candidate_bus
    ...
    # Now reassemble them (the bus stops that survived + the non-bus from the feed)
    ...
    # The key columns we'll use for final consolidation:
